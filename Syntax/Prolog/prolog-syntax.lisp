...
#|
(climacs1-gui::define-named-command com-inspect-lex ()
  (with-slots (lexer) (slot-value (buffer (climacs1-gui::current-window)) 'drei-syntax::syntax)
    (let ((*standard-input* *query-io*)
	  (*standard-output* *query-io*))
      (inspect lexer))))

(climacs1-gui::define-named-command com-inspect-parse ()
  (with-slots (parser) (slot-value (buffer (climacs1-gui::current-window)) 'drei-syntax::syntax)
    (let ((*standard-input* *query-io*)
	  (*standard-output* *query-io*))
      (inspect parser))))
|#
