...
(clim:define-command
    (com-multiple-query-replace-from-buffer :name t
                                            :command-table drei:search-table)
  ...
  (let* ((buffer (esa-io:buffer view))
         (contents (drei-buffer:buffer-substring
                    buffer
                    0
                    (1- (drei-buffer:size buffer))))
         (strings (loop with length = (length contents)
                        with index = 0
                        with start = 0
                        while (< index length)
                        do (loop until (>= index length)
                                 while (drei-syntax:whitespacep
                                        (drei-syntax:syntax buffer)
                                        (char contents index))
                                 do (incf index))
                           (setf start index)
                           (loop until (>= index length)
                                 until (drei-syntax:whitespacep
                                        (drei-syntax:syntax buffer)
                                        (char contents index))
                                 do (incf index))
                        until (= start index)
...
(clim:define-command
    (com-query-exchange :name t :command-table drei:search-table)
    ()
...
(defun multiple-query-replace-set-key (gesture command)
  (clim:add-command-to-command-table
   command 'multiple-query-replace-drei-table
   :keystroke gesture
   :errorp nil))
