...
of a structural object."
  (let ((immediate-form (funcall immediate-form-fn (drei:current-syntax)
        ...
        (form-around (form-around (drei:current-syntax)
                                  (drei-buffer:offset (drei:point))))
        (list-at-mark (list-at-mark (drei:current-syntax) (drei:point)))
        (string-at-mark (form-of-type-at-mark (drei:current-syntax) (drei:point) #'form-string-p)))
...
S-expression, however, not zero."
  (if (in-string-p (drei:current-syntax) (drei:point))
      (setf (drei-buffer:offset (drei:point))
            (1+ (end-offset (form-around (drei:current-syntax) (drei:point)))))
...
forward into the enclosing list."
  (let ((list (list-at-mark (drei:current-syntax) (drei:point))))
    (when list
...
after the point."
  (let ((form-around (form-around (drei:current-syntax)
        ...
        (form-after (form-after (drei:current-syntax)
                                (drei-buffer:offset (drei:point))))
        (comment (comment-at-mark (drei:current-syntax) (drei:point))))
    (cond ((drei-base:empty-line-p (drei:point))
          ...
          ((in-string-p (drei:current-syntax) (drei:point))
           (if (= (drei-buffer:buffer-line-number
               ...
               (drei-base:kill-region (drei:point) (1- (end-offset form-around)))
               ;; Delete from point until end of line.
               (drei-base:kill-region
                (drei:point)
                (drei-buffer:end-of-line (drei-buffer:clone-mark (drei:point))))))
          ((in-line-comment-p (drei:current-syntax) (drei:point))
           ;; Delete until end of line
           (drei-base:kill-region
            (drei:point)
            (drei-buffer:end-of-line (drei-buffer:clone-mark (drei:point)))))
          ((in-long-comment-p (drei:current-syntax) (drei:point))
           (if (= (drei-buffer:buffer-line-number
                   (esa:current-buffer)
                   (end-offset comment))
                  (drei-buffer:line-number (drei:point)))
               ...
               (drei-base:kill-region (drei:point)
                                      (- (end-offset comment)
