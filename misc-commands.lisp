...
(clim:define-command (com-not-modified :name t :command-table climacs1-gui:buffer-table) ()
...
(clim:define-command (com-what-cursor-position :name t :command-table drei:info-table) ()
...
FIXME: gives no information at end of buffer."
  (let* ((char (or (drei-buffer:end-of-buffer-p (drei:point)) (drei-buffer:object-after (drei:point))))
    ...
    (esa:display-message "Char: ~:[none~*~;~:*~:C (#o~O ~:*~D ~:*#x~X)~] point=~D of ~D (~D%) column ~D"
                     ...
                     (drei-buffer:offset (drei:point)) (drei-buffer:size (esa:current-buffer))
...
(clim:define-command (com-browse-url :name t :command-table climacs1-gui:base-table)
...
(clim:define-command (com-set-syntax :name t :command-table climacs1-gui:buffer-table)
...
(clim:define-command (com-define-group :name t :command-table climacs1-gui:global-climacs-table)
    ((name 'string :prompt "Name")
     (views '(sequence clim:view) :prompt "Views"))
  (when (or (not (climacs-core:get-group name))
            (clim:accept 'boolean :prompt "Group already exists. Overwrite existing group?"))
    (climacs-core:add-group name views))
  (climacs-core:select-group (climacs-core:get-group name)))

(esa:set-key `(com-define-group ,clim:*unsupplied-argument-marker* ,clim:*unsupplied-argument-marker*)
         'climacs1-gui:global-climacs-table
         '((#\x :control) (#\g) (#\d)))

(clim:define-command (com-define-file-group :name t :command-table climacs1-gui:global-climacs-table)
    ((name 'string :prompt "Name")
...
(esa:set-key `(com-define-file-group ,clim:*unsupplied-argument-marker* ,clim:*unsupplied-argument-marker*)
         'climacs1-gui:global-climacs-table
         '((#\x :control) (#\g) (#\f)))

(clim:define-command (com-select-group :name t :command-table climacs1-gui:global-climacs-table)
...
(clim:define-command (com-deselect-group :name t :command-table climacs1-gui:global-climacs-table)
    ()
...
(clim:define-command (com-current-group :name t :command-table climacs1-gui:global-climacs-table)
    ()
...
(clim:define-command (com-list-group-contents :name t :command-table climacs1-gui:global-climacs-table)
    ()
  (esa:with-minibuffer-stream (s)
